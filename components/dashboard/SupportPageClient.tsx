      ...
      <div className="w-full sm:max-w-lg rounded-t-2xl sm:rounded-2xl border border-white/10 bg-brand-bg p-4 sm:p-6 space-y-5 max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-3 pt-1">
            <Button type="button" onClick={onClose} className="flex-1 h-10 rounded-xl bg-white/5 hover:bg-white/10 text-white/60 hover:text-white border border-white/8 text-sm font-medium">
              Cancel
            </Button>
            <Button
              type="submit"
    ...
    <>
      ...
      <div className="space-y-8 max-w-4xl">
        <div className="flex items-start justify-between gap-4">
          ...
          <Button onClick={() => setShowNew(true)} className="flex items-center gap-2 h-9 px-4 rounded-xl bg-brand-accent hover:bg-brand-accent-hover text-brand-bg text-sm font-bold shadow-lg shadow-brand-accent/20 shrink-0">
        ...
        {tickets.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 gap-4 text-center">
            ...
            <Button onClick={() => setShowNew(true)} className="flex items-center gap-2 h-9 px-4 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-white text-sm font-medium">
              <Plus className="w-3.5 h-3.5" />
              Open a ticket
            </Button>
          </div>
              ...
              >
                <div className="flex-1 min-w-0 space-y-1.5">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className={cn('text-[10px] font-medium border rounded-full px-2 py-0.5', STATUS_COLORS[ticket.status])}>
